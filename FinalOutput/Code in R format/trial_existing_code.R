...
readData <- function(){
  ...
  ratingDF <- read.delim("~/Works/Rworks/Movielens/Analytics/ml-100k/u.data", header=F)
  colnames(ratingDF) <- c("userID","movieID","rating", "timestamp")
  
  ## read movie data
  moviesDF <- read.delim("~/Works/Rworks/Movielens/Analytics/ml-100k/u.item", sep="|", header=F, stringsAsFactors = FALSE)
